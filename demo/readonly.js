...
For more info, visit https://reactjs.org/link/mock-scheduler`);
  ...
  var nodeWidth = 156;
  var nodeMarginX = 4;
  var nodeHeaderHeight = 24;
  var portRadius = 12;
  var portMargin = 12;
  var portHeight = portRadius * 2 + portMargin * 2;
  var canvasPaddingRight = 480;
  var inputPortArc = `a ${portRadius} ${portRadius} 0 0 1 0 ${2 * portRadius}`;
  var inputPort = `v ${portMargin} ${inputPortArc} v ${portMargin}`;
  function signalInvalidType(type) {
    console.error(type);
    throw new Error("Unexpected type");
  }
  function makeClipPath(kinds2, kind) {
    const { inputs, outputs } = kinds2[kind];
  ...
  function place({ options }, { x, y }, offset) {
    if (offset === void 0) {
  ...
  function getKeyIndex(ports, name) {
    ...
    if (indices !== void 0) {
      ...
      const indices2 = Object.fromEntries(keys.map((key, index) => [key, index]));
      keyIndexCache.set(ports, indices2);
      return indices2[name];
    }
  }
  var getInputIndex = (kinds2, kind, input) => getKeyIndex(kinds2[kind].inputs, input);
  var getOutputIndex = (kinds2, kind, output) => getKeyIndex(kinds2[kind].outputs, output);
  function getInputOffset(kinds2, kind, input) {
    const index = getInputIndex(kinds2, kind, input);
    return [0, getPortOffsetY(index)];
  }
  function getOutputOffset(kinds2, kind, output) {
    const index = getOutputIndex(kinds2, kind, output);
    return [nodeWidth, getPortOffsetY(index)];
  }
  function* forInputs(kinds2, kind) {
    for (const input of Object.keys(kinds2[kind].inputs)) {
      yield input;
    }
  }
  function* forOutputs(kinds2, kind) {
    for (const output of Object.keys(kinds2[kind].outputs)) {
      yield output;
    }
  }
  var minCurveExtent = 104;
  ...
  function GraphInput(props) {
    ...
    const ref = (0, import_react3.useCallback)((circle) => {
      if (circle !== null && props.inputDrag) {
        select_default2(circle).call(props.inputDrag);
      }
    }, []);
    ...
    const isFocused = props.focus !== null && props.focus.element === "edge" && props.focus.id === value;
    return import_react3.default.createElement("g", { className: isFocused ? "input focus" : "input", "data-id": props.node.id, "data-input": props.input, "data-value": value, transform, strokeWidth: isFocused ? 3 : void 0 }, import_react3.default.createElement("text", { stroke: "none", x: portRadius + nodeMarginX, dominantBaseline: "middle" }, props.input), import_react3.default.createElement("circle", { ref, className: "port", cursor: "grab", display: value === null ? "none" : "inherit", fill: backgroundColor, r: portRadius, onClick: handleClick }));
  }
  ...
  function GraphOutput(props) {
    ...
    const { backgroundColor } = context.options;
    const ref = (0, import_react4.useCallback)((circle) => {
      if (circle !== null && props.outputDrag) {
        select_default2(circle).call(props.outputDrag);
      }
    }, []);
    const values = props.node.outputs[props.output];
    const isFocused = props.focus !== null && props.focus.element === "edge" && values.includes(props.focus.id);
    return import_react4.default.createElement("g", { className: isFocused ? "output focus" : "output", "data-id": props.node.id, "data-output": props.output, transform, strokeWidth: isFocused ? 3 : void 0 }, import_react4.default.createElement("text", { textAnchor: "end", stroke: "none", x: -portRadius - nodeMarginX, dominantBaseline: "middle" }, props.output), import_react4.default.createElement("circle", { ref, className: "port", cursor: "grab", fill: backgroundColor, r: portRadius }));
  }

  // lib/Node.js
  function GraphNode(props) {
    const { name, backgroundColor, inputs, outputs } = props.kinds[props.node.kind];
    const clipPath = (0, import_react5.useMemo)(() => makeClipPath(props.kinds, props.node.kind), []);
    const context = (0, import_react5.useContext)(CanvasContext);
    const ref = (0, import_react5.useCallback)((g) => {
      if (g !== null && props.nodeDrag) {
        select_default2(g).call(props.nodeDrag);
      }
    }, []);
    const handleClick = (0, import_react5.useCallback)((event) => {
      context.onFocus({ element: "node", id: props.node.id });
    }, []);
    const transform = toTranslate(place(context, props.node.position));
    const isFocused = props.focus !== null && props.focus.element === "node" && props.focus.id === props.node.id;
    const { borderColor } = context.options;
    return import_react5.default.createElement("g", { ref, className: isFocused ? "node focus" : "node", "data-id": props.node.id, "data-kind": props.node.kind, "data-position-x": props.node.position.x, "data-position-y": props.node.position.y, stroke: borderColor, strokeWidth: isFocused ? 3 : 1, transform, cursor: "grab", onClick: handleClick }, import_react5.default.createElement("path", { fill: backgroundColor, d: clipPath }), import_react5.default.createElement("text", { stroke: "none", x: 8, y: 18 }, name), props.children, import_react5.default.createElement("line", { stroke: borderColor, strokeWidth: 1, x1: nodeMarginX, y1: nodeHeaderHeight, x2: nodeWidth - nodeMarginX, y2: nodeHeaderHeight }), import_react5.default.createElement("g", { className: "inputs" }, Object.keys(inputs).map((input) => import_react5.default.createElement(GraphInput, { key: input, kinds: props.kinds, focus: props.focus, node: props.node, input, inputDrag: props.inputDrag }))), import_react5.default.createElement("g", { className: "outputs", transform: "translate(156, 0)" }, Object.keys(outputs).map((output) => import_react5.default.createElement(GraphOutput, { key: output, kinds: props.kinds, focus: props.focus, node: props.node, output, outputDrag: props.outputDrag }))));
  }

  // lib/Edge.js
  var import_react6 = __toModule(require_react());
  function GraphEdge(props) {
    const { kinds: kinds2, nodes, edge } = props;
    const source = nodes[edge.source.id];
    const target = nodes[edge.target.id];
    const context = (0, import_react6.useContext)(CanvasContext);
    const sourcePosition = (0, import_react6.useMemo)(() => {
      const offset = getOutputOffset(kinds2, source.kind, edge.source.output);
      return place(context, source.position, offset);
    }, [edge.source, source.position]);
    const targetPosition = (0, import_react6.useMemo)(() => {
      const offset = getInputOffset(kinds2, target.kind, edge.target.input);
      return place(context, target.position, offset);
    }, [edge.target, target.position]);
    const path = (0, import_react6.useMemo)(() => makeCurvePath(sourcePosition, targetPosition), [
      sourcePosition,
      targetPosition
    ]);
    const handleClick = (0, import_react6.useCallback)((event) => {
      context.onFocus({ element: "edge", id: props.edge.id });
    }, []);
    const { borderColor, backgroundColor } = context.options;
    const isFocused = props.focus !== null && props.focus.element === "edge" && props.focus.id === props.edge.id;
    return import_react6.default.createElement("g", { className: isFocused ? "edge focus" : "edge", strokeWidth: isFocused ? 12 : 8, "data-id": edge.id, "data-source-id": edge.source.id, "data-source-kind": source.kind, "data-source-output": edge.source.output, "data-target-id": edge.target.id, "data-target-kind": target.kind, "data-target-input": edge.target.input, cursor: "pointer", onClick: handleClick }, import_react6.default.createElement("path", { stroke: borderColor, fill: "none", d: path }), import_react6.default.createElement("path", { strokeWidth: 6, stroke: backgroundColor, fill: "none", d: path }));
  }

  // lib/styles.js
  var import_react7 = __toModule(require_react());
  var getCanvasStyle = ({ borderColor }) => ({
    borderColor,
    borderWidth: 1,
    borderStyle: "solid",
    width: "100%",
  ...
  var getSVGStyle = ({ borderColor, unit, height }) => ({
  ...
  var defaultStyleContext = { getCanvasStyle, getSVGStyle };
  var StyleContext = import_react7.default.createContext(defaultStyleContext);
  function useStyles() {
    const { options } = (0, import_react7.useContext)(CanvasContext);
    const { getCanvasStyle: getCanvasStyle2, getSVGStyle: getSVGStyle2 } = (0, import_react7.useContext)(StyleContext);
    return (0, import_react7.useMemo)(() => ({ canvas: getCanvasStyle2(options), svg: getSVGStyle2(options) }), []);
  }

  // lib/reduce.js
  function reduce(kinds2, state, action) {
    if (action.type === "node/create") {
      const { id, kind, position } = action;
      const nodes = { ...state.nodes };
      nodes[id] = createInitialNode(kinds2, id, kind, position);
      ...
      for (const input of forInputs(kinds2, kind)) {
        const edgeId = inputs[input];
        if (edgeId !== null) {
          const { source: { id: sourceId, output } } = edges[edgeId];
          delete edges[edgeId];
          const source = nodes[sourceId];
          const outputs2 = new Set(source.outputs[output]);
          outputs2.delete(edgeId);
          nodes[sourceId] = {
            ...source,
            outputs: { ...source.outputs, [output]: Array.from(outputs2) }
          };
        }
      }
      for (const output of forOutputs(kinds2, kind)) {
      ...
      delete nodes[id];
      const focus2 = isFocusEqual(state.focus, { element: "node", id }) ? null : state.focus;
      return { ...state, nodes, edges, focus: focus2 };
      ...
      delete edges[id];
      const focus2 = isFocusEqual(state.focus, { element: "edge", id }) ? null : state.focus;
      return { ...state, edges, nodes, focus: focus2 };
  ...
  function createInitialNode(kinds2, id, kind, position) {
    const inputs = Object.fromEntries(Object.keys(kinds2[kind].inputs).map((input) => [input, null]));
    const outputs = Object.fromEntries(Object.keys(kinds2[kind].outputs).map((output) => [output, []]));
  ...
  // lib/viewer/ReadonlyCanvas.js
  var import_react10 = __toModule(require_react());
  function Canvas2(props) {
    const context = (0, import_react10.useContext)(CanvasContext);
    const styles = useStyles();
    const width = (0, import_react10.useMemo)(() => {
      return getCanvasWidth(context, props.state.nodes);
    }, [props.state.nodes]);
    const handleBackgroundClick = (0, import_react10.useCallback)((_) => {
    ...
    return import_react10.default.createElement("div", { className: "canvas", style: { ...styles.canvas, boxShadow } }, import_react10.default.createElement("svg", { ref: context.svgRef, xmlns: "http://www.w3.org/2000/svg", style: { ...styles.svg, minWidth: width }, height: unit * height }, import_react10.default.createElement("rect", { className: "background", fill: "transparent", height: unit * height, style: { minWidth: width, width: "100%" }, onClick: handleBackgroundClick }), import_react10.default.createElement("g", { className: "edges", ref: context.edgesRef }, Object.values(props.state.edges).map((edge) => import_react10.default.createElement(GraphEdge, { key: edge.id, kinds: props.kinds, nodes: props.state.nodes, focus: props.state.focus, edge }))), import_react10.default.createElement("g", { className: "nodes", ref: context.nodesRef }, Object.values(props.state.nodes).map((node) => import_react10.default.createElement(GraphNode, { key: node.id, kinds: props.kinds, focus: props.state.focus, node })))));
  }

  // lib/viewer/Viewer.js
  function Viewer(props) {
    const dispatchRef = (0, import_react11.useRef)(props.dispatch);
    dispatchRef.current = props.dispatch;
    const dispatch = (0, import_react11.useCallback)((action) => dispatchRef.current(action), []);
    const focusRef = (0, import_react11.useRef)(props.state.focus);
    ...
    return import_react11.default.createElement(CanvasContext.Provider, { value: context }, import_react11.default.createElement(Canvas2, { kinds: props.kinds, state: props.state, dispatch }));
  }
  ...
  function Index({
    ...
    const dispatch = (0, import_react12.useCallback)((action) => {
      ...
      setState(reduce(kinds2, stateRef.current, action));
    }, []);
    return /* @__PURE__ */ import_react12.default.createElement(Viewer, {
      kinds: kinds2,
      state,
      dispatch
    });
  }
  import_react_dom.default.render(/* @__PURE__ */ import_react12.default.createElement(Index, {
    kinds,
    initialState: {
      focus: null,
      nodes: {
